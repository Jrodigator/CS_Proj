...
import menu
# import game objects
from Objects import Bee, Lives, Enemy, Bullets, Stinger, Barrier, Projectile, Jellies
# import random 
...
class Game:
    ...
    def process(self):
        ...
        for i in self.Ant:
                ...
                if i.collider(j):
                    if i.hitPoints:
                        ...
                        if i.hitPoints < 0.5:
                            ...
                            #drop jelly
                            if Jellies.Spawn():
                                self.Jellies.append(Jellies(i.pos))
                        else: 
                            self.Bullets.remove(j)
            ...
            try :
                if i.collider(self.Bee):
                    try: self.Lives.pop(len(self.Lives) - 1) ; self.Bee.hitPoints -= 1;  self.Ant.remove(i)
                    except : pass
            except: pass
        ...
        for i in self.Birds:
            ...
            # Bird Bee Collision
            if i.collider(self.Bee):
                try: self.Lives.pop(len(self.Lives) - 1) ; self.Bee.hitPoints -= 1 ;  self.Birds.remove(i)
                except : pass
        ...
        # jelly Bee Collisions
        for i in self.Jellies:
            if i.collider(self.Bee):
                jelly = Jellies.Collect(i)
    ...
    #All Pause Menu Functionality Implemented by John Cokayne 25863878
    def pause_menu(self):
        for event in pygame.event.get():
            if event.type==pygame.QUIT:
                    pygame.quit()
