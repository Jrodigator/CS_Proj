...
import highscore as leaderboard

#Implemented by John Cokayne 25863878 with a few lines by Jared Rodrigues
#All background music also implemented by John Cokayne

class Title_Screen:
    #initialises all the components of the title screen
    def __init__(self):
        self.__init__pygame()
    ...
    #draws the title screen using the screens components
    ...
    #loops while the user is on the title_screen
    def title_loop(self):
        while True:

            #checks if any background music is playing. If not, it starts playing Retro_Platforming.wav on a loop
            if not pygame.mixer.get_busy():
                bg_music = pygame.mixer.Sound("Assets\Retro_Platforming.wav")
                bg_music.play(loops = -1)

            #checks if the user pressed the quit button and quits if true. Also checks if they pressed the space button and jumps into the instruction screen loop if true.
...
class Instruction_Screen:
    ...
    #intialises the components of the instruction screen
    def __init__(self):
        self.__init__pygame()
        self.screen = pygame.display.set_mode((696, 528))
        self.title_font = pygame.font.Font("Assets/minecraftfont.otf",50)
...
#intialises pygame
    ...
    def __init__pygame(self):
        pygame.init()
        pygame.display.set_caption("Protect The Hive")
    #draws the components of the instruction screen to the screen surface. if self.help is true, displays the page showing the jelly information
    def draw(self):
        if self.help:
            self.screen.fill((254, 200, 9))
        ...
        pygame.display.flip()
            #self.clock.tick(60)
#loops while the user is on the instruction screen
        ...
        while True:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pygame.quit()
                    quit()
                #check if the space bar is pressed and if so, jumps into the menu loop and starts displaying the main menu
                if event.type==pygame.KEYDOWN and event.key==pygame.K_SPACE:
                    instance=Menu()
                    instance.menu_loop()
                #checks if the user clciks on the arrow at the bottom of the screen. if so, the screen starts displaying the jelly info page if its on the instruction page and vice versa
            ...
            self.draw()
class Menu:
    #initialises the menu screen components
    def __init__(self):
        self.__init__pygame()
        self.screen = pygame.display.set_mode((696, 528))
    ...
    #displays the components of the menu screen
    def draw(self):
    ...
    #loops while the user is on the menu screen
    def menu_loop(self):
        while True:
            #checks if any music is playing. if not, starts playing Retro_Platforming.wav
            if not pygame.mixer.get_busy():
                bg_music = pygame.mixer.Sound("Assets\Retro_Platforming.wav")
                bg_music.play(loops = -1)
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pygame.quit()
                    quit()
                #checks if the user clicked on any of the buttons on the main menu screen and if so, calls the method that will start displaying the appropriate screen
                if event.type==pygame.MOUSEBUTTONUP:
                    mouse_pos=pygame.mouse.get_pos()
...
class Levels:
        #initialises the components of the level select screen
        def __init__(self):
        ...
        def draw(self):
            ...
            pygame.display.flip()
            self.clock.tick(60)
        #loops while the user is on the level select screen
        def level_loop(self):
            back=False
            while True:
                for event in pygame.event.get():
                    ...
                    if event.type==pygame.MOUSEBUTTONUP:
                        mouse_pos=pygame.mouse.get_pos()
                        #checks if one of the levels is slelcted by clicking on it. if so, starts the game on that level
                        if self.level1.collidepoint(mouse_pos):
                            ...
                            instance.game_loop()
                            print("level 5")
                        #checks if the user pressed the back button. if so, takes the user back to the main menu
                        elif  self.back.collidepoint(mouse_pos):
                            back=True
                if back: break
                self.draw()

'''Pretty sure this code is no longer used. keeping it in just incase it is necessary, we can easily reimplement it.
class Help:
        def __init__(self):
            pygame.init()
            pygame.display.set_caption("Protect The Hive")
